...
chrome.extension.onRequest.addListener(function (request, sender, sendResponse) {
    ...
    case 'entire':
        ...
        break;
    case 'https':
        alert('出于安全原因, “截取选择部分”的功能不能在https页面上使用！请尝试其他选项.');
    ...
    case 'insert_script':
        ...
        chrome.tabs.executeScript(tabid, {
                file: 'javascripts/content_script.js'
            },
            function () {
                sendRequest('tab', tabid, {
...
chrome.runtime.onInstalled.addListener(function (o) {
    if (o.reason == 'install') {
        chrome.tabs.create({
            url: "http://www.awesomescreenshot.com/welcome/awesome_screenshot.html"
        });
    }
});
// 1. let all people see 'New': don't detect 'isClickedOnNew' and remove 'isClickedOnNew' from localStorage
// 2. small update after big update , don't want to people who has seen what's new page to see again: detect 'isClickedOnNew', don't remove 'isClickedOnNew'
/*if (typeof localStorage['version'] == "undefined") {
    chrome.tabs.create({url: "http://www.awesomescreenshot.com/welcome/awesome_screenshot.html"});
} else*/
if ( /*!localStorage['isClickedOnNew'] &&*/ localStorage['version'] != currentversion) {
